	...
	<body>
		...
		<h2>오라클 접속하기</h2>
				...
				<sql:setDataSource driver="oracle.jdbc.driver.OracleDriver" 
					url="jdbc:oracle:thin:@192.168.100.129:1521:XE" user="hr" password="hr" var="ds"/>
		...
		<p>JDBC 프로그래밍시 DB관련 작업에서 연결객체 생성하고, 소멸하는 작업이 빈번하게 일어나는데
		   이때 접속자가 많으면 실행시간이 많이 지연됨</p>
		<p>따라서, 연결객체를 만든 후 바로 소멸하지 않고, 이 객체들을 저장하는 저장소pool에 일시적으로 저장해 두었다가
		다른 사용자가 사용해야 할 경우 객체를 다시 생성하지 않고 기존에 만들어둔 객체를 재사용하게 함</p>
		<p>DBCP를 사용하려면 DB접속정보를 설정해야 하는데, 이것을 datasource라 하고 JNDI 표기법을 이용해서 정의함 </p>
		...
		자바 프로그램에서 네트워크 URI로 모든 자원의 위치를 파악하고 필요시 그것을 쉽게
		호출하기 위해 사용하는 규칙 
		</p>
		<p>DB 접속 정보 작성 - datasource 정의 <br>
		conf/server.xml에 Context 태그 속에 resource 태그로 작성하고,
		WEB-INF/web.xml 에 resource-ref 태그로 datasource를 참조
		</p>	
		...
		<!-- 		 jdbc:mysql://아이피주소:3306/데이터베이스이름 -->
		<!-- mariadb : org.mariadb.jdbc.Driver-->
		<!--          jdbc:mariadb://아이피주소:3306/데이터베이스이름 -->
		
		<sql:setDataSource var="mysql" driver="com.mysql.jdbc.Driver" 
		url="jdbc:mysql://192.168.189.129:3306/yeogiyong" user="yeogiyong" password="Abcdef_12"/>
		<sql:query var="rs" dataSource="${mysql}">
			select city, state, phone from customers
		</sql:query>
